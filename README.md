...
*   [x] Service 层单元测试（25 用例：移动防环/配额记账/回收站级联/秒传去重/版本历史/分享）
*   [x] 网盘 M3（v2 八大功能，详见 [docs/FEATURES.md](docs/FEATURES.md)）：
    *   [x] 文件在线预览（图片/PDF/音视频/文本，`?inline=1`）
    *   [x] 分享链接（提取码 + 有效期 + 下载计数 + 公开访问页）
    *   [x] 批量操作（多选删除/移动/zip 打包下载）
    *   [x] 回收站 30 天自动清理（启动时 + 每小时扫描）
    *   [x] 秒传 / 去重存储（SHA-256 命中即创建条目，物理对象引用计数共享）
    *   [x] 分片 / 断点续传（8MB 分片、断点查询、合并校验哈希）
    *   [x] 文件版本历史（同名上传自动留旧版、可恢复、版本组级联删除）
    *   [x] 操作日志 / 最近动态（全量操作时间线）
*   [ ] Docker 容器化部署支持
